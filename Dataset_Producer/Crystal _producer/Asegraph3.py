...
def struct2graph(structure, jobname, maxnodes,y_atoms=None,  AtomSymbol=False,
                 properties_list=['geometry', 'mass', 'id', 'electronegativity'],num_tasks=1):
    struct = structure.copy()
    y_atoms_state = np.zeros((maxnodes,num_tasks), dtype=np.float32)
    atom_features_list = np.zeros((maxnodes, 1), dtype=np.int64)
    y_marking = np.zeros((maxnodes,num_tasks), dtype=np.float32)
    for i, atom in enumerate(struct):
        atom_features_list[i] = struct.get_atomic_numbers()[i]
        if y_atoms is not None:
            y_atoms_state[i][:] = y_atoms[i]
            y_marking[i][:] = 1
